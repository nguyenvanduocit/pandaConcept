---
name: style-mediterranean-greek
description: "Greek Mediterranean Interior Design interior design style — detailed reference with colors (hex), materials, furniture, AI rendering keywords, and room applications"
---

# Greek Mediterranean Interior Design

---

## Philosophy
Celebrate the sun, the sea, and the whitewashed clarity of Cycladic tradition. Greek Mediterranean design is the most distilled of the Mediterranean variants — architecture bleached to mineral purity, punctuated by the deep blue of the Aegean. Thick plastered stone walls evolved for climate: cool interiors in intense heat, small deep-set windows controlling solar gain. The palette is not chosen — it is borrowed directly from the landscape and sea.

---

## Core Characteristics
1. Cycladic whitewash — smooth plaster over stone, bleached by sun to pure mineral white
2. Deep blue accents (Aegean, cobalt, indigo) referencing sea and sky
3. Minimal furniture against bold architectural backdrop — architecture is the decoration
4. Stone (local limestone, marble) as primary structural and decorative material
5. Geometric floor mosaics in terracotta and white
6. Iron and bronze hardware — hand-forged, organic forms
7. Indoor-outdoor courtyard integration (*aule*)
8. Whitened wood beams on ceilings
9. Arched niches for ceramics and religious icons
10. Natural light through small, deep-set windows to control heat
11. Blue-and-white patterned ceramics (Delphi, Aegean ware)
12. Open-air sleeping platforms (*therinaion*) as transitional spaces

---

## Materials

| Material | Specifics | Application |
|----------|-----------|-------------|
| Hymettian marble | Pale grey-white veined | Traditional Attica stone surfaces |
| Pentelic white marble | Bright white | Classical building stone, flooring, sculpture |
| Naxian emery stone | Dark volcanic | Flooring and feature surfaces |
| Local limestone | Rough-cut natural stone | Structural walls, floor paving, rough-cut steps |
| Whitewashed plaster | Smooth lime over stone, repeated annually | All wall and ceiling surfaces |
| Wrought iron | Hand-forged | Window grilles, furniture legs, candleholders |
| Hand-thrown terracotta | Locally crafted, unglazed | Pots, storage vessels, floor tiles |
| Aegean blue glazed tiles | Deep blue ceramic glaze | Kitchen and bath splashbacks |
| Driftwood | Natural, weathered | Sculptural objects, picture frames |
| Sailcloth and undyed linen | Crisp, natural, sun-faded | Curtains, seat cushions |

---

## Color Palette
| Color Name | Hex Code | Usage Context |
|---|---|---|
| Cycladic White | `#F4F1EC` | Walls, dominant all surfaces |
| Aegean Blue | `#1A5276` | Shutters, door frames, accents |
| Cobalt Sky | `#2E86C1` | Ceramic tiles, textiles |
| Terracotta | `#C0714A` | Floors, pots, accent |
| Sea-Bleached Linen | `#E8DCC8` | Textiles, soft furnishings |
| Iron Grey | `#5D6D7E` | Metal work, stone secondary |
| Sand | `#D4B483` | Stone surfaces, gravel paths |
| Olive Leaf | `#6D7C4A` | Plant accent, ceramic glaze |

---

## Signature Furniture
- Low-slung whitewashed bench built into wall (*krevati*)
- Blue-painted wooden chair with rush seat (traditional taverna form)
- Marble-top wrought iron table (courtyard and interior)
- Carved limestone niche shelf for ceramics and icons
- Woven rush-seat stool — local craft tradition
- Hammered copper vessel as decorative accent

---

## Lighting
- Simple wrought iron candelabra with beeswax candles
- Hand-blown glass pendants in pale amber or clear
- Oil lamp reproductions in terracotta or bronze
- Natural light the primary source — deep-set windows create dramatic shafts
- Exterior torch-style wall sconces flanking doorways
- 2700K–3000K where artificial light is used

---

## Textiles & Patterns
- Undyed sailcloth — crisp, natural, sun-faded
- Blue-and-white woven stripe (*flokati* influence)
- Embroidered linen (Greek cross-stitch, geometric motifs)
- Natural linen slipcovers — unlined, unadorned
- Minimal pattern — architecture carries all visual interest

---

## Architectural Elements
- Engawa (transitional veranda/corridor) connecting interior to garden
- Tokonoma — replaced here by arched niche for icon or ceramic display
- Thick (40–60cm) stone or rubble walls — thermal mass
- Barrel-vaulted ceilings in cave-house tradition (Santorini)
- Flat roof with whitewashed parapet
- Blue-painted wooden shutters and doors
- External staircase as sculptural architectural element
- Courtyard (*aule*) with central cistern or lemon tree

---

## Room Applications
- **Living:** White plaster walls, minimal low furniture, blue accent shutters, terracotta floor, iron chandelier
- **Bedroom:** Whitewashed platform bed built into niche, blue linen, small high window, single ceramic lamp
- **Kitchen:** Open stone shelf, terracotta pots, hand-painted tiles behind sink, limestone worktop
- **Bathroom:** Marble basin, whitewashed walls, terracotta floor, simple iron fittings, no vanity cabinet

---

## 2024–2025 Trends
- "New Cycladic" — luxury resort hotels driving ultra-refined version globally
- Curved cave-form architecture (Santorini type) influencing non-Greek interiors
- Greek blue palette entering contemporary European and American design
- Limewash plaster as accessible alternative to traditional whitewash
- Outdoor living spaces referencing Greek terrace and courtyard culture

---

## AI Rendering Keywords
`greek mediterranean interior, cycladic white washed walls, aegean blue shutters greek, greek island living room whitewash, arched niche ceramics greek, terracotta floor greek island, wrought iron chandelier greek, blue door white plaster greek, santorini cave house interior, greek courtyard fountain, marble basin greek bathroom, blue and white ceramic tile, blue painted wooden shutters, domed ceiling greek island, minimal greek interior sunlight`

---

## Complementary Styles
Rustic, Bohemian, Tropical, Coastal

---

## Avoid
Heavy ornament, dark heavy furniture, maximalist layering, non-natural materials, bright non-blue colors, carpeted floors

---

## Greek Regional Sub-Styles

### Cycladic (Santorini, Mykonos, Paros)
The purest expression: cave-formed architecture, barrel-vaulted ceilings, no straight lines — all walls rounded by generations of hand-plastered lime. Furniture built into the architecture (beds, benches, shelves plastered smooth). Minimal possessions. The blue dome of the Oia church is the global icon. Interior colors: pure white + Aegean blue only.

### Mainland Greek / Athenian
More urban, more layered. Neoclassical influence from the 19th century (Greek independence era). Marble floors, formal proportions, iron balconies, shuttered windows. Less spartan than Cycladic; more architectural detail.

### Greek Island (Rhodes, Crete, Corfu)
Each island has distinct character: Rhodes has Ottoman and Italian overlay (Knights of Rhodes era); Crete has Minoan reference and heavier carved wood; Corfu has Venetian influence — loggias, pastel facades. More color than pure Cycladic.

---

## Seasonal & Climate Logic
- Small windows (30–50cm deep reveals) minimize solar gain in summer
- White exterior reflects 80%+ of solar radiation — functional, not purely aesthetic
- Courtyard orientation captures north breeze while shading south face
- Stone floor absorbs coolness at night; releases it slowly through day
- These climate responses are what make the Greek aesthetic: function is form

---

## Craft Traditions
- **Komboloi** (worry beads) — amber, wood, or stone; displayed as sculptural object
- **Blue eye (mati)** — evil eye amulet in blue glass; hung at entrance
- **Hand-thrown terracotta** — Aegina, Sifnos, Crete; each island's clay is distinct
- **Embroidery** — Rhodes embroidery (red and black on white linen); Skyrian embroidery (polychrome)
- **Carved marble** — traditional gravestone and architectural detail craft

---

## Scent & Sensory Design
- Wild thyme, oregano, and sage growing on terraces — scent enters through windows
- Beeswax candles — honey scent, warm amber light
- Saltwater air — the defining environmental quality that all Greek interiors absorb
- Lemon and orange blossom from courtyard trees — seasonal fragrance calendar
